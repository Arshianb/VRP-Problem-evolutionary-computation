            ...
            if self.pop_size - int(self.pop_size/dived_into) < len(temp_fitness):
                ...
                for i in range(len(self.fitnesses)-1):
                    if abs(self.fitnesses[last_unchange_index] - self.fitnesses[i+1])<0.01:
                        ...
                        if diff < 50:
                            self.pop[i+1][:len(self.pop[i+1])-11] = Inversion().Mutate(self.pop[i+1][:len(self.pop[i+1])-11], 1)
                            self.pop[i+1][len(self.pop[i+1])-11:] = self.Mutation_obj1(self.pop[i+1][len(self.pop[i+1])-11:], 1)
        ...
        while (iters < 5000):
            ...
            for _ in range(self.pop_size):
                if np.random.random() < Pc:
                    ...
                    child1, child2 = Recombination_obj.Recombination(parent_1[:len(parent_1)-11], parent_2[:len(parent_1)-11])
                    child1_, child2_ = Recombination_obj_depotes(parent_1[len(parent_1)-11:], parent_2[len(parent_1)-11:])
                    child1.extend(child1_)
                    child2.extend(child2_)
                    # print(child1)
                    child1 = [x-1 for x in child1] 
                    child2 = [x-1 for x in child2] 
                    child1_1 = self.Mutation_obj.Mutate(child1[:len(child1)-11], Pm)
                    child2_1 = self.Mutation_obj.Mutate(child2[:len(child2)-11], Pm)
                    child1_2 = self.Mutation_obj1(child1[len(child1)-11:], Pm)
                    child2_2 = self.Mutation_obj1(child2[len(child1)-11:], Pm)

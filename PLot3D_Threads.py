...
def Problem2_plot(Main_path = "Problem 1\\best chromosome"):
    ...
    plt.show()
def Problem3_plot(Main_path = "Problem 1\\best chromosome"):
    X = []
    Y = []
    Z = []
    Z_avg = []
    ...
    for csv_File in glob.glob("{}/*".format(Main_path)):
        ...
        for row in csv_row:
            ...
            Y.append(int(csv_File[csv_File.find("number")+len("number"):csv_File.find(".csv")]))
            Z.append(float(row[1]))
            XYDemand = XYZ_("Data/P3.txt")
            Z_avg.append(float(row[2]))
            if index == whole:
                vehicle = 1
                vehicles_last_position = [[-17, -4]] * 4
                all_distance = 0
                fitness_distance = 0
                howmany_ = 0
                next_vehicle = False
                    ...
                    if vehicle <= 4:
                        while True:
                            if not next_vehicle:
                                all_distance = all_distance + evaluate_distance(vehicles_last_position[vehicle-1], [XYDemand[0][gene], XYDemand[1][gene]])
                                if all_distance <= 200:
                                    Fitness = Fitness + XYDemand[2][gene] 
                                vehicles_last_position[vehicle-1] = [XYDemand[0][gene], XYDemand[1][gene]]

                                if all_distance + evaluate_distance(vehicles_last_position[vehicle-1], [-17, -4]) > 200:
                                    next_vehicle = True
                                    all_distance = 0    
                                else:
                                    break
                                ...
                                next_vehicle = False
                                vehicles_last_position[vehicle-1] = [-17, -4]
                                all_distance = 0
                                vehicle+=1
                                if vehicle == 5:
                                    break
...
Problem3_plot(Main_path = "Problem 3\\best chromosome")